...
2. **Schema + admin** — migrations for the configuration tables (`app_configs`, `agents`, Active Storage; session tables land with slice 3), models + validations, seeds (3 placeholder agents, default config, admin from env), `rails admin:create`, signup disabled in production unless `ALLOW_SIGNUP`, Alba serializers, `/api/admin/*` endpoints, React admin pages (settings form, agent form with avatar upload and voice select), ElevenLabs voice list endpoint with Solid Cache.
